...
function renderHero(active: string) {
  ...
  <div class="herorow">
    <section class="hero">
      <div class="chiprow">
        <span class="meta-chip">Crop compliance</span>
      ...
      <h1>Crop compliance and field-observation control plane.</h1>
      <p>Observation queues, evidence blockers, and field posture in one operator surface for agri teams that need cleaner buyer and regulator handoffs before review windows burn.</p>
      ${renderTabs(active, links)}
    </section>
    <aside class="side">
      <div class="bluf">
        <div class="lbl">Current watch item</div>
        <p><strong>Spray-log parity, scout-sheet corrections, and acreage drift</strong> are the fastest ways to turn a healthy crop packet into a burned buyer or program review window.</p>
      </div>
      <div class="corr">
        <div class="lbl">Operator correction</div>
        <p>Map every observation set to evidence, owner, and audience-safe posture before growers, compliance leads, and reviewers start working from different crop narratives.</p>
      </div>
      <div class="toolchips">
...
function heroShell(active: string, content: string) {
  return `${renderHero(active)}${content}
    <footer>
      <div>crop-compliance-observation-ledger · synthetic demonstration data only</div>
  ...
  return layout(
    ...
    heroShell(
      ...
      `<section class="section">
        <div class="sh"><h2>Control snapshot</h2><div class="note">Crop-safe agri ops</div></div>
        <div class="kpis">
          <div class="kpi cyan"><div class="v">${metrics.observations}</div><div class="lbl">Observation cases</div><div class="h">Active field observation cases tied to crop, owner, and next action.</div></div>
          <div class="kpi red"><div class="v">${metrics.urgentObservations}</div><div class="lbl">Urgent observations</div><div class="h">Red cases where buyer or regulator review posture is already under pressure.</div></div>
          <div class="kpi amber"><div class="v">${metrics.blockedCompliance}</div><div class="lbl">Blocked compliance</div><div class="h">Evidence and packet blockers still waiting on proof.</div></div>
          <div class="kpi plum"><div class="v">${metrics.fragileFieldPackets}</div><div class="lbl">Fragile field packets</div><div class="h">Field-facing packets that still need stronger audit posture before release.</div></div>
        </div>
        <div class="quote">
          <div class="lbl">Primary recommendation</div>
          <div class="q">${metrics.recommendation}</div>
          <div class="a">Crop trust comes from aligned field notes, proof bundles, and reviewer-safe packet posture.</div>
          <div class="qm">”</div>
        </div>
      </section>
      <section class="section">
