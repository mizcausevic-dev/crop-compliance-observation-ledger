import {
  ...
  summary,
  verification
} from "./cropComplianceObservationLedgerService";
...
<head>
  ...
  <title>${title}</title>
  <meta name="description" content="TypeScript control plane for crop observations, evidence blockers, field posture, and buyer-safe agri compliance operations. Operator surface in the Kinetic Gain Suite." />
  ...
  <link rel="canonical" href="https://crops.kineticgain.com/" />
  <link rel="icon" type="image/svg+xml" href="${KG_FAVICON_DATA_URI}" />
  <meta property="og:type" content="website" />
  <meta property="og:site_name" content="Kinetic Gain" />
  <meta property="og:title" content="${title}" />
  <meta property="og:url" content="https://crops.kineticgain.com/" />
  <meta property="og:description" content="TypeScript control plane for crop observations, evidence blockers, field posture, and buyer-safe agri compliance operations. Operator surface in the Kinetic Gain Suite." />
  <meta name="twitter:card" content="summary" />
  <meta name="twitter:title" content="${title}" />
  <meta name="twitter:description" content="TypeScript control plane for crop observations, evidence blockers, field posture, and buyer-safe agri compliance operations. Operator surface in the Kinetic Gain Suite." />
  <script type="application/ld+json">{"@context":"https://schema.org","@graph":[{"@type":"ProfessionalService","@id":"https://crops.kineticgain.com/#service","name":"Kinetic Gain — TypeScript control plane for crop observations, evidence blockers, field posture, and buyer-safe agri compliance operations","url":"https://crops.kineticgain.com/","areaServed":"United States","description":"TypeScript control plane for crop observations, evidence blockers, field posture, and buyer-safe agri compliance operations. Operator surface in the Kinetic Gain Suite."},{"@type":"WebSite","@id":"https://crops.kineticgain.com/#website","url":"https://crops.kineticgain.com/","name":"TypeScript control plane for crop observations, evidence blockers, field posture, and buyer-safe agri compliance operations","publisher":{"@type":"Organization","name":"Kinetic Gain LLC"}}]}</script>
  <style>${KG_STYLE01_CSS}${KG_COMPAT_CSS}${MENU_SURFACE_CSS}</style>
</head>
<body>
  <header>
    <div class="wrap nav">
      <a class="kg-logo" href="/" aria-label="Kinetic Gain — TypeScript control plane for crop observations, evidence blockers, field posture, and buyer-safe agri compliance operations">
        ${KG_MARK_SVG}
        <span class="kg-word">Kinetic Gain</span>
      </a>
      <nav class="nav-links" id="primaryNav">
        <a href="/observation-lane">Observation Lane</a>
        <a href="/compliance-risks">Compliance Risks</a>
        <a href="/field-posture">Field Posture</a>
    ...
    <div class="wrap">
      <div class="foot-top">
        <div>
          ...
          <p class="foot-tag">TypeScript control plane for crop observations, evidence blockers, field posture, and buyer-safe agri compliance operations — operator surface in the Kinetic Gain Suite. Static demo data; no live calls.</p>
        </div>
        <div class="foot-cols">
          <div class="foot-col">
            <h4>Estate</h4>
          ...
          <div class="foot-col">
            <h4>This surface</h4>
            <a href="https://github.com/mizcausevic-dev/crop-compliance-observation-ledger">GitHub repo</a>
          </div>
        </div>
      </div>
      <div class="foot-bot">
...
function renderHero(active: string) {
  const links: NavLink[] = [
    { href: "/", label: "Overview" },
    { href: "/observation-lane", label: "Observation Lane" },
    { href: "/compliance-risks", label: "Compliance Risks" },
    { href: "/field-posture", label: "Field Posture" },
    { href: "/verification", label: "Verification" },
    { href: "/docs", label: "Docs" }
  ];

  const metrics = summary();
  const urgentCase = observationLane().find((item) => item.risk === "red") ?? observationLane()[0];
  const complianceRisk = complianceRisksLane().find((item) => item.readiness === "red") ?? complianceRisksLane()[0];
  const packet = fieldPosture().find((item) => item.status !== "green") ?? fieldPosture()[0];
  return `${topbar(active)}
  <section class="hero reveal">
    <div class="acard hero-panel">
      <div class="hero-grid">
        <div class="hero-copy">
          <span class="eyebrow">Crop compliance</span>
          <div class="hero-meta">
            <span class="meta-chip">Field observations</span>
            <span class="meta-chip">Buyer-safe evidence</span>
            <span class="meta-chip">Operator escalation</span>
          </div>
          <h1>Where do field observations quietly become buyer and regulator risk?</h1>
          <p class="hero-subtle">Crop Compliance Observation Ledger keeps spray-log parity, scout-sheet corrections, acreage drift, and audit-safe packet posture visible before the next buyer review or regulator window burns.</p>
          <div class="hero-nav">${routePills(active, links)}</div>
          <div class="hero-kpis">
            <div class="stat"><label>Observations</label><strong>${metrics.observations}</strong><span>Active field observation cases tied to crop, owner, and next move.</span></div>
            <div class="stat"><label>Urgent observations</label><strong>${metrics.urgentObservations}</strong><span>Red cases where buyer or regulator posture is already under pressure.</span></div>
            <div class="stat"><label>Blocked compliance</label><strong>${metrics.blockedCompliance}</strong><span>Proof gaps still waiting on field, lab, or program evidence.</span></div>
            <div class="stat"><label>Fragile packets</label><strong>${metrics.fragileFieldPackets}</strong><span>Field packets still too weak to send through audit safely.</span></div>
          </div>
        </div>
        <aside class="hero-aside">
          <div class="acard">
            <span class="metric-chip">Current watch item</span>
            <h3>${urgentCase.farm} · ${urgentCase.crop}</h3>
            <p><strong>${urgentCase.region}</strong><br />${urgentCase.excerpt}</p>
          </div>
          <div class="acard">
            <span class="metric-chip">Operator correction</span>
            <h3>What the team should do next</h3>
            <ul class="hero-mini-list">
              <li><strong>${urgentCase.owner}</strong><span>${urgentCase.nextAction}</span></li>
              <li><strong>${complianceRisk.owner}</strong><span>${complianceRisk.requiredEvidence}</span></li>
              <li><strong>${packet.audience}</strong><span>${packet.decisionNote}</span></li>
            </ul>
          </div>
          <div class="acard">
            <span class="metric-chip">Control surfaces</span>
            <p>Field notes, spray logs, residue evidence, acreage math, and reviewer-safe packet posture stay in one readable view instead of drifting across growers, compliance, and program reviewers.</p>
          </div>
        </aside>
      </div>
    </div>
...
export function renderOverview() {
  const metrics = summary();
  const cases = observationLane();
  const risks = complianceRisksLane();
  const packets = fieldPosture();
  return layout(
    "Crop Compliance Observation Ledger",
    heroShell(
      "/",
      `<section class="section">
        <div class="sh"><h2>Observation lane</h2><div class="note">Field, residue, and acreage pressure that leadership can actually act on</div></div>
        <div class="section-cards">
          ${cases.map((item) => `<article class="acard surface-card"><span class="tag ${item.risk}">${item.risk}</span><h3>${item.farm} · ${item.crop}</h3><div class="surface-meta"><p><strong>${item.caseId}</strong> · ${item.region}</p><p><strong>Owner:</strong> ${item.owner}</p></div><p class="surface-copy">${item.excerpt}</p><p class="surface-next"><strong>Next move:</strong> ${item.nextAction}</p></article>`).join("")}
        </div>
      </section>
      <section class="section">
        <div class="sh"><h2>Compliance pressure</h2><div class="note">Where evidence, acreage, and narrative drift can still break audit trust</div></div>
...
export function renderObservationLane() {
  const rows = observationLane();
  return layout(
    "Crop Compliance Observation Ledger — Observation Lane",
    heroShell(
      "/observation-lane",
      `<section class="section">
        <div class="sh"><h2>Observation lane</h2><div class="note">Cases under pressure</div></div>
        <table class="ttbl">
          <thead><tr><th>Observation case</th><th>Owner</th><th>Risk</th><th>Next move</th></tr></thead>
          <tbody>
            ${rows
              .map(
                (item) => `<tr>
                  <td><b>${item.farm} · ${item.crop}</b><br />${item.caseId} · ${item.region}<br />${item.excerpt}</td>
                  <td>${item.owner}</td>
...
export function renderComplianceRisks() {
  const risks = complianceRisksLane();
  return layout(
    "Crop Compliance Observation Ledger — Compliance Risks",
    heroShell(
      "/compliance-risks",
      `<section class="section">
        <div class="sh"><h2>Compliance risks</h2><div class="note">Evidence gaps before buyer and regulator promises</div></div>
        <div class="ledger">
          <div class="led">
...
export function renderFieldPosture() {
  const packets = fieldPosture();
  return layout(
    "Crop Compliance Observation Ledger — Field Posture",
    heroShell(
      "/field-posture",
      `<section class="section">
        <div class="sh"><h2>Field posture</h2><div class="note">Packet confidence and reviewer timing</div></div>
        <div class="board">
          ${packets
            .map(
...
export function renderVerification() {
  ...
  return layout(
    "Crop Compliance Observation Ledger — Verification",
    heroShell(
      "/verification",
      `<section class="section">
        ...
        <div class="ledger">
          ...
          <div class="led">
            ...
            <ul>
              <li><span class="pip">→</span><div>Crop observations, evidence blockers, and review timing can surface in-product without exposing unsafe write paths.</div></li>
              <li><span class="pip">→</span><div>Buyer-safe field posture stays connected to dashboards, operator reviews, and launch-safe remediation workflows.</div></li>
              <li><span class="pip">→</span><div>Synthetic data only; this repo demonstrates system shape, not production grower or buyer records.</div></li>
            </ul>
          </div>
        </div>
      </section>`
...
export function renderDocs() {
  return layout(
    "Crop Compliance Observation Ledger — Docs",
    heroShell(
      "/docs",
      `<section class="section">
        <div class="sh"><h2>System docs</h2><div class="note">Routes and control surfaces</div></div>
        <div class="stack">
          <article class="src"><div class="src-name">Control plane</div><div class="src-tit">Observation lane</div><p>Case-by-case field queue with crop, region, owner routing, and next crop-safe move.</p></article>
          <article class="src"><div class="src-name">Risk map</div><div class="src-tit">Compliance risks</div><p>Required evidence, owner lanes, and impact areas for blocked buyer, export, and program posture.</p></article>
          <article class="src"><div class="src-name">Field view</div><div class="src-tit">Field posture</div><p>Packet-level confidence, blocker state, and next reviewer-window timing.</p></article>
          <article class="src"><div class="src-name">Embedded tie-back</div><div class="src-tit">KG Embedded</div><p>Security-first in-product analytics extension for crop compliance, field review cadence, and grower-safe workflows.</p></article>
        </div>
        <div class="quote">
          <div class="lbl">Route surface</div>
          <div class="q"><code>/</code> · <code>/observation-lane</code> · <code>/compliance-risks</code> · <code>/field-posture</code> · <code>/verification</code> · <code>/docs</code></div>
